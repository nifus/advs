  ...
  function advPreviewDirective($interval, $cookies, $filter) {
    ...
    function advPreviewController($scope) {
      ...
      $scope.openDefaultBlock = function () {
        $scope.env.display = $scope.adv.photos.length>0 ? 'photos' : 'map';
      ...
      function initPhotoBlock() {
        ...
        function initDom() {
          var el = $('#photo-block');
          m_images = $('#photo-block div.main img');
          p_images = $('#photo-block div.preview div.preview-item');

          navigate = $('#photo-block div.navigate');
          back_navigate = $('#photo-block div.back');
          next_navigate = $('#photo-block div.next');
          m_images.on('load', function (e) {
            $('#photo-block div.main div.center').css('max-width', m_images.width() + 'px')
          });
      ...
      $scope.$watch('env.display', function () {
        console.log($scope.env.display)
      })
    }
